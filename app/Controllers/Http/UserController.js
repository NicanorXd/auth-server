...
class UserController {
  index = async ({ request }) => {
    ...
    let users = User.query()
      ...
      .where("users.state", currentState);
    // filtro ids
    if (query_search)
      users.whereRaw(
        `(email like '%${query_search || ""}%' OR username like '%${
          query_search || ""
        }%' OR p.fullname like '%${query_search}%')`
      );
    if (ids.length) users.whereIn("users.id", ids);
    // paginatión
    users = await users.select("users.*").paginate(page || 1, 20);
    // add imagen
  ...
  async update({ params, request }) {
    ...
    if (is_username)
      throw new ValidatorError([
        { field: "username", message: `El username ya está en uso` },
      ]);
    // validar email
  ...
  state = async ({ params, request }) => {
    try {
      ...
      return {
        success: true,
        code: 201,
        message: `La cuenta de "${user.email}" se ${
          state ? "restauró" : "desactivo"
        } correctamente!`,
      };
    } catch (error) {
      return {
  ...
  block = async ({ params, request }) => {
    ...
    if (query_search)
      methods.whereRaw(
        `(methods.name LIKE '%${query_search}%' OR methods.url LIKE '%${query_search}%' OR sys.name LIKE '%${query_search}%')`
      );
    if (block == 1)
      methods.whereRaw(
        `EXISTS (SELECT null FROM block_method_users WHERE user_id = ${params.id} AND methods.id = method_id)`
      );
    else
      methods.whereRaw(
        `NOT EXISTS (SELECT null FROM block_method_users WHERE user_id = ${params.id} AND methods.id = method_id)`
      );
    // paginar
  ...
  dependencia = async ({ params, request, response }) => {
    ...
    if (except) {
      ...
      dependencia
        .join(
          "config_entity_dependencias as conf_dep",
          "conf_dep.dependencia_id",
          "dependencias.id"
        )
        ...
        .select(
          "dependencias.*",
          "conf_dep.id as config_entity_dependencia_id"
        );
    }
    // get entities
